...
def gen_image(
    ...
    resp = client.images.generate(
        prompt=prompt, model="gpt-image-1", size="auto", output_format="png", n=n
    )
    ...
    for image in resp.data:
        ...
        logger.info(
            f'<img src="{Path(file_name).name}" alt="Generated image" />', html=True
        )

        b64 = image.b64_json
        bb = base64.b64decode(b64)
        Path(file_name).write_bytes(bb)
